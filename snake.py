...
class square:
    ...
    def set_vel(self,new_x,new_y):
        self.vel_x = self.new_x
        self.vel_y = self.new_y
    ...
    def update (self):
        if self.x>0 and self.x<width - grid_size:
            self.x+=self.vel_x
        if self.y>0 and self.x<heigh - grid_size:
            self.y+=self.vel_y
        if self.x==0 and self.vel_x > 0:
            self.x+=self.vel_x    
        if self.x== width - grid_size and self.vel_x < 0:
            self.x+=self.vel_x    

        if self.y==0 and self.vel_y > 0:
            self.y+=self.vel_y    
        if self.y== heigh - grid_size and self.vel_y < 0:
            self.y+=self.vel_y  
...
class game :
    def __init__(self) :
        ...
        f = square (random.randint(grid_size,int(width/grid_size))*grid_size-grid_size,random.randint(grid_size,int(heigh/grid_size))*grid_size-grid_size,'purple')
    ...
    def run(self):
        ...
        while True:
            ...
            for i in range (len(self.vel)):
                self.snake[i].vel_x = self.vel [i][0]
                self.snake[i].vel_x = self.vel [i][1]
